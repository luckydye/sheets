...
export class Model {
     ...
     * @param {number} delta
     */
    moveColumn(sheet, column, delta) {
        const ret = wasm.model_moveColumn(this.__wbg_ptr, sheet, column, delta);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
     ...
     * @param {number} delta
     */
    moveRow(sheet, row, delta) {
        const ret = wasm.model_moveRow(this.__wbg_ptr, sheet, row, delta);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
    ...
    setSheetColor(sheet, color) {
        ...
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.model_setSheetColor(this.__wbg_ptr, sheet, ptr0, len0);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
